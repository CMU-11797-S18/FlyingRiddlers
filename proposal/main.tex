...
My primary hypothesis is that the architecture of a well-performing Question Answering system would be highly dependent on and specific to the dataset (size and nature), beyond tuning of model hyper-parameters. More specifically, for smaller datasets, traditional IR techniques and extractive summarization methods would yield better performance while for larger datasets, neural ranking algorithms and abstractive text summarization would work better. I am also keen on exploring the interdepedency of the different modules in the QA pipeline, and understand the behaviour of different combinations of specific modules. 

It is with these goals that I propose to implement the following techniques:

\begin{itemize}
\item \textbf{Query Processing}

I wish to explore the effect of variants of query processing techniques such as stemming, POS tagging, normalization and query term expansion on the quality of the retrieved snippets. I would also experiment with different query restructuring and expansion methods using pseudo relevance feedback and term expansion.
...
Since abstractive summarization methods typically tend to yield low ROUGE-N scores and extractive models have poor human readibility, I intend to work on developing a model that's a cross between the two, with a loss function to optimize for more meaningful and smoother transitions while retaining many n-grams from the extracted sentences. At the same time, I am also interested in exploring more advanced extractive methods based on neural architectures and graphical models.
...
% \begin{longtable}{@{\,}r <{\hskip 2pt} | r !{\foo} >{\raggedright\arraybackslash}p{5cm}}
